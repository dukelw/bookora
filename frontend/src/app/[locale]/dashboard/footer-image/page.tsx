...
export default function FooterImageManagementPage() {
  ...
  const [sortStatus, setSortStatus] = useState<"all" | "active" | "inactive">(
    "all"
  );
      ...
      if (val) {
        const nextOrder =
          activeImages.length > 0
            ? Math.max(...activeImages.map((i) => i.order ?? 0)) + 1
            : 1;
        await footerImageService.update(img._id!, {
          isActive: true,
          order: nextOrder,
        });
      ...
      <div className="p-6 shadow rounded-2xl">
          ...
          >
            {options.map((opt) => (
              <DropdownItem
                key={opt.value}
                onClick={() =>
                  setSortStatus(opt.value as "all" | "active" | "inactive")
                }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {pagedImages.map((img) => (
            <div
              key={img._id}
              className="bg-gray-50 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col"
            >
              <div
