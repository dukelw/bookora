...
const BookDropdownItem = ({ book, onSelect, t }: { book: Book; onSelect: (b: Book) => void; t: any }) => (
  <div
  ...
  >
    ...
    <div className="text-sm text-gray-600">
      {t("p.author")}: {book.author} | {t("p.publisher")}: {book.publisher} | {t("p.releaseYear")}: {book.releaseYear}
    </div>
    <div className="text-xs text-blue-600 mt-1">{book?.variants?.length} {t("i.availableVariants")}</div>
  </div>
);
    ...
    <div>
      ...
      <div className="p-6 shadow rounded-2xl">
        <div className="relative mb-6" ref={searchRef}>
          <label className="block text-sm font-medium text-gray-700 mb-2">{t("i.searchBook")}</label>
          ...
          {showDropdown && filteredBooks.length > 0 && (
            ...
            >
              {filteredBooks.map((book) => (
                <BookDropdownItem key={book._id} book={book} onSelect={handleBookSelect} t={t} />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 mb-8">
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6 rounded-t-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-blue-100">
              <div><span className="font-medium">{t("p.author")}:</span> {selectedBook.author}</div>
              <div><span className="font-medium">{t("p.publisher")}:</span> {selectedBook.publisher}</div>
              <div><span className="font-medium">{t("p.releaseYear")}:</span> {selectedBook.releaseYear}</div>
            </div>
          </div>

          <div className="p-6">
            <div className="mb-6">
              <h3 className="font-semibold text-gray-700 mb-2">{t("p.description")}:</h3>
                ...
                <table className="w-full border-collapse bg-gray-50 rounded-lg overflow-hidden">
                  <thead>
                    <tr className="bg-gray-200">
                      <th className="border border-gray-300 px-4 py-3 text-left font-semibold text-gray-700">{t("p.category")}</th>
                      <th className="border border-gray-300 px-4 py-3 text-center font-semibold text-gray-700">{t("i.image")}</th>
                      <th className="border border-gray-300 px-4 py-3 text-left font-semibold text-gray-700">{t("p.price")}</th>
                      <th className="border border-gray-300 px-4 py-3 text-left font-semibold text-gray-700">{t("p.stock")}</th>
                      <th className="border border-gray-300 px-4 py-3 text-left font-semibold text-gray-700">ISBN</th>
                    </tr>
                  </thead>
                  <tbody>
                    {selectedBook?.variants?.map((variant, idx) => (
                      <tr key={variant._id} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        ...
                        <td className="border border-gray-300 px-4 py-3">
                          <span
                            className={`font-medium ${
                              variant.stock < 50 ? "text-red-600" : variant.stock < 200 ? "text-yellow-600" : "text-green-600"
                            }`}
                          >
                            {variant.stock.toLocaleString()} cuốn
                          </span>
                        </td>
                        <td className="border border-gray-300 px-4 py-3 text-gray-600">{variant.isbn ?? "Không có"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
